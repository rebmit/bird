...
nl_parse_link(struct nlmsghdr *h, int scan)
{
  ...
  if (a[IFLA_LINKINFO])
  {
	...
	ea_set_attr_u32(&f.attrs, &ea_iface_bridge_vlan_filtering, 0, vlan_filtering);
      }
    }
    else if (!bstrcmp(kind, "vxlan") && li[IFLA_INFO_DATA])
    {
      ea_set_attr_u32(&f.attrs, &ea_iface_type, 0, IF_TYPE_VXLAN);
	...
	ea_set_attr_u32(&f.attrs, &ea_iface_vxlan_id, 0, id);
      }

      if (data[IFLA_VXLAN_LEARNING])
      {
	u8 learning = rta_get_u8(data[IFLA_VXLAN_LEARNING]);
	ea_set_attr_u32(&f.attrs, &ea_iface_vxlan_learning, 0, learning);
      }
...
nl_parse_fdb(struct nl_parse_state *s, struct nlmsghdr *h)
{
  ...
  kbr_got_fdb(p, &n, new ? &e0 : NULL, nhad, src, s->scan, tunnel);
}
